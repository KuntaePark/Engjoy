...
wss.on("connection", (ws) => {
  const gameState = new GameState(); //GameState 객체 생성
  setupLevel(gameState);

  const player = gameState.addPlayer();
  const playerId = player.id;
  ws.playerId = playerId; //ws 객체에 playerId 저장

  console.log(`[Connection] Player ${playerId} has connected.`);

  //클라이언트(각 플레이어)에게 ID 알려주기
  ws.send(JSON.stringify({ type: "playerId", payload: playerId }));
  console.log(`[Message Sent] Sent 'yourId' to ${playerId}.`);
  ...
  ws.on("message", (data) => {
    ...
    switch (message.type) {
      case "input":
        ...
        break;
      case "interact":
        gameState.playerHoldingKeywords(ws.playerId);
        break;
    }
  });
